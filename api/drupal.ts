...
const fetchAllUserIssues = async (userId: string, startTimestamp: number | null = null): Promise<DrupalIssue[]> => {
  ...
  while (currentUrl) {
    ...
    if (issues.length > 0) {
      ...
      if (startTimestamp && lastIssue.created < startTimestamp) {
          break;
      }
...
const fetchIssuesWithUserComments = async (userId: string, year = 2025): Promise<DrupalIssue[]> => {
  ...
  while (currentUrl) {
    ...
    if (comments.length > 0) {
      ...
      // Stop condition: If the last comment on the page is older than the start date, we can stop fetching.
      if (comments[comments.length - 1].created < startDate) {
        break;
      }
